...
#   --clear-queues     Truncate .backlog queues (events_queue, fetch_queue, process_queue)
...
PIPELINE="$ROOT_DIR/scripts/pipeline/pipeline_manager.sh"
BACKLOG_WORKER="$ROOT_DIR/scripts/pipeline/backlog_worker_manager.sh"
...
usage() {
  cat <<'EOF'
Usage: scripts/pipeline/restart_agents.sh [--clear-logs] [--clear-queues] [--renew-baseline]
...
if [[ $CLEAR_QUEUES -eq 1 ]]; then
  ...
  : > "$BACKLOG_DIR/fetch_queue_internal.txt"
  : > "$BACKLOG_DIR/fetch_queue_external.txt"
