...
HELPER="$ROOT_DIR/scripts/helpers/fetch_all_cursus.sh"
EXPORT_DIR="$ROOT_DIR/exports/cursus"
...
jq -r '.[] | [
  ...
  | run_psql -c "\copy cursus_delta (id,name,slug,kind,created_at,raw_json) FROM STDIN WITH (FORMAT csv)"

echo "Upserting cursus..."
run_psql <<'SQL'
WITH upsert AS (
  INSERT INTO cursus (id,name,slug,kind,created_at,raw_json)
  SELECT id,name,slug,kind,created_at,raw_json::jsonb FROM cursus_delta
